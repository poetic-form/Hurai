                ...
                VStack(alignment: .leading, spacing: 30) {
                    VStack(alignment: .leading, spacing: 24) {
                        VStack(alignment: .leading, spacing: 22) {
                            ...
                            Text("본 앱은 사용자에게 알림 권한 및 스크린타임 연동 권한을 요청하며, 일부 비식별 정보(usage 및 진단 데이터)를 수집합니다. 또한 Mixpanel을 통해 기기 ID(개인 식별 불가), 대략적 위치 정보(정밀 위치 제외), 앱 내 제품 상호작용 정보(화면 열림, 버튼 클릭 등)가 분석 목적으로 수집될 수 있습니다. 이 데이터는 모두 비식별화된 형태로 처리되며, 사용자를 특정하거나 개인 정보를 유추하는 데 사용되지 않습니다.\n\n본 앱을 사용하는 경우, 아래의 개인정보 처리방침에 동의하는 것으로 간주합니다.".byCharWrapping)
                    ...
                    VStack(alignment: .leading, spacing: 22) {
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            Text("후라이는 사용자의 식별이 불가능한 비식별 사용 데이터를 수집할 수 있습니다. 이 데이터는 어떤 기능이 얼마나 사용되는지 등을 파악해 앱의 성능을 개선하고 사용자 경험을 향상시키기 위한 용도로만 사용됩니다.\n또한 Mixpanel을 통해 기기 ID(개인 식별 불가), 대략적 위치 정보(정밀 위치 제외), 앱 내 제품 상호작용 정보(화면 열림, 버튼 클릭 등)가 분석 목적으로 수집될 수 있습니다. 이 데이터는 모두 비식별화된 형태로 처리되며, 사용자를 특정하거나 개인 정보를 유추하는 데 사용되지 않습니다. 수집된 데이터는 마케팅, 광고, 데이터 마이닝 등 다른 목적에는 절대 사용되지 않으며, 제3자에게 제공되지 않습니다.".byCharWrapping)
                                .pretendard(.medium, 16)
                        }
                    ...
                    VStack(alignment: .leading, spacing: 22) {
                        ...
                        Text("모든 데이터는 사용자의 기기 내에 로컬로 저장되며, 개발자 서버 등 외부 저장소에 전송되지 않습니다.\n다만, Mixpanel 분석에 필요한 비식별 데이터는 Mixpanel 서버에 저장될 수 있으며, 해당 데이터는 Mixpanel의 개인정보 보호 정책에 따라 안전하게 관리됩니다. 앱 삭제 시 모든 데이터는 자동으로 삭제됩니다.".byCharWrapping)
                            .pretendard(.medium, 16)
                    }
                    ...
                    VStack(alignment: .leading, spacing: 22) {
                        ...
                        Text("후라이는 서버에 개인정보를 저장하지 않기 때문에, 별도의 장기 보관 기간이 적용되지 않습니다.\n모든 데이터는 사용자의 기기에만 존재하며, 앱 삭제 시 자동으로 파기됩니다.\nMixpanel을 통해 수집된 비식별 분석 데이터는 서비스 개선 목적 범위 내에서 보관될 수 있습니다.".byCharWrapping)
                            .pretendard(.medium, 16)
                    }
